...
*/
  function addNewToDo(newItem) {
    ...
    //  template literal and convert to an element node
    let documentFragment = document.createRange().createContextualFragment(markup);
    // read it back as element node
    let elements = documentFragment.querySelector('li')
    // add events to the element within the element node.
    let trashIcon = elements.querySelector('img')
    console.log(elements)
   trashIcon.addEventListener('click', onRemoveItem)
     toDoContainer.appendChild(elements)
  }
